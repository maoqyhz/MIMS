...
{
    public class PHA_DosageFormDAL
    {
        ...
        public IList GetList()
        {
            using (Conn)
            {
                string query = "SELECT * FROM PHA_DosageForm";
                return Conn.Query<PHA_DosageForm>(query).ToList();
            }
        ...
        public IList GetList(string where)
        {
            using (Conn)
            {
                string query = "SELECT * FROM PHA_DosageForm WHERE 1=1 AND";
                query += where;
                return Conn.Query<PHA_DosageForm>(query).ToList();
            }
